...
int main(int argc, char *argv[])
{
    ...
    if (size <= 2)
    {
        ...
        printf("Must be more then 2 proc\n");
...
void n_body_par(int argc, char *argv[])
{
    ...
    if (rank == MASTER)
    {
        ...
        for (int i = 1; i < size; i++)
        {
            ...
            MPI_Send(&realChunk, 1, MPI_INT, i, TAG_CHUNK, MPI_COMM_WORLD);
            MPI_Send(&chunkMod, 1, MPI_INT, i, TAG_CHUNK, MPI_COMM_WORLD);
            MPI_Send(&chunkSize, 1, MPI_INT, i, TAG_CHUNK, MPI_COMM_WORLD);
        }
        ...
        for (step = 1; step <= n_steps; step++)
        {
            ...
            for (int i = 0; i < size - 1; i++)
            {
                ...
                int currentSize;
                if (currProc == size - 1)
                    currentSize = chunkSize + chunkMod;
                else
                    currentSize = chunkSize;

                MPI_Recv((curr + chunkSize * i), currentSize, particletype, currProc, TAG_GATHER, MPI_COMM_WORLD, &status);
            }
            ...
            for (int i = 0; i < size - 1; i++)
            {
                ...
                for (int j = currProc - 1; j >= 1; j--)
                {
                    MPI_Send((curr + chunkSize * i), currentSize, particletype, j, TAG_PARTICLES, MPI_COMM_WORLD);
                }
            }
            ...
            for (int i = 0; i < size - 2; i++)
            {
                ...
                for (int j = currProc + 1; j < size; j++)
                {
                    ...
                    MPI_Recv(forces + chunkSize * (j - 1), currentSize, forcetype, currProc, TAG_GATHER, MPI_COMM_WORLD, &status);

                    MPI_Send(forces + chunkSize * (j - 1), currentSize, forcetype, j, TAG_FORCES, MPI_COMM_WORLD);
                }
            }
    ...
    else if (rank > MASTER)
    {
        ...
        MPI_Recv(&realChunk, 1, MPI_INT, MASTER, TAG_CHUNK, MPI_COMM_WORLD, &status);
        MPI_Recv(&chunkMod, 1, MPI_INT, MASTER, TAG_CHUNK, MPI_COMM_WORLD, &status);
        MPI_Recv(&chunkSize, 1, MPI_INT, MASTER, TAG_CHUNK, MPI_COMM_WORLD, &status);

        curr = malloc(realChunk * sizeof(struct particle_s));
        forces = malloc(realChunk * sizeof(vect_t));

        curr_received = malloc((chunkSize + chunkMod) * sizeof(struct particle_s));
        forces_received = malloc((chunkSize + chunkMod) * sizeof(vect_t));
        ...
        while (1)
        {
            ...
            MPI_Send(curr, realChunk, particletype, MASTER, TAG_GATHER, MPI_COMM_WORLD);
            ...
            if (rank < size - 1)
            {
                for (int i = rank + 1; i < size; i++)
                {
                    ...
                    MPI_Recv(curr_received, (chunkMod + chunkSize), particletype, MASTER, TAG_PARTICLES, MPI_COMM_WORLD, &status);

                    if (i == size - 1)
                    {
                        for (int part = 0; part < realChunk; part++)
                            Compute_received_force(&(forces[part]), forces_received, curr[part], curr_received, (chunkMod + chunkSize));
                        MPI_Send(forces_received, chunkSize + chunkMod, forcetype, MASTER, TAG_GATHER, MPI_COMM_WORLD);
                    }
                    else
                    {
                        for (int part = 0; part < realChunk; part++)
                            Compute_received_force(&(forces[part]), forces_received, curr[part], curr_received, realChunk);
                        MPI_Send(forces_received, realChunk, forcetype, MASTER, TAG_GATHER, MPI_COMM_WORLD);
                    }
                }
            }
            ...
            for (int i = 1; i < rank; i++)
            {
                ...
                MPI_Recv(forces_received, realChunk, forcetype, MASTER, TAG_FORCES, MPI_COMM_WORLD, &status);
            ...
            MPI_Recv(&end, 1, MPI_INT, MASTER, TAG_END, MPI_COMM_WORLD, &status);
